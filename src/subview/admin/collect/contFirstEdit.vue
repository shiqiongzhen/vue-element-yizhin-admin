...
export default {
  ...
  methods: {
      initData(page) {
        this.$http.post(this.$api.First_search,this.Qs.stringify({
          "categoryF.categFirstId":this.$route.params.cataid
        }))
       .then((response) => { 
          if(response.data.errorMessage == "您还未登录，请先登录"){
            ...
            // console.log(response)
              this.form = response.data.categoryFirsts[0]
              if(response.data.categoryFirsts[0].isTask) this.type.push("任务分类")
              if(response.data.categoryFirsts[0].isService) this.type.push("服务分类")
              if(response.data.categoryFirsts[0].isHelp) this.type.push("帮助分类")
              if(response.data.categoryFirsts[0].isNews) this.type.push("新闻分类")
          }
        })
      ...
      submitForm(formName) {
            this.$http.post(this.$api.First_update,
            this.Qs.stringify({
              "categoryF.categFirstId": this.$route.params.cataid,
              "categoryF.categName": this.form.categName,
              "categoryF.isTask": this.type.indexOf("任务分类") > -1?1:0,
              "categoryF.isService":  this.type.indexOf("服务分类") > -1?1:0,
              "categoryF.isHelp":  this.type.indexOf("帮助分类") > -1?1:0,
              "categoryF.isNews":  this.type.indexOf("新闻分类") > -1?1:0,
              "categoryF.winBidTaskFee": this.form.winBidTaskFee
            }))
            .then((response) => {
