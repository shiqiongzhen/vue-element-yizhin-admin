    ...
    <div class="search_container">
      <el-form :inline="true"  class="demo-form-inline">
        ...
        <el-form-item style="float: right;">
          <router-link to="/main/prov/add"><el-button type="primary" icon="el-icon-plus"></el-button></router-link>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{scope.row.provCategoryId}}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="	名称">
...
export default {
  ...
  methods: {
    cataSearch() {
        ...
        this.$http.post(this.$api.Prov_search,
            this.Qs.stringify({
              "provCategoryJ.provCategoryId": this.searchList[0].keyword,
              "provCategoryJ.provName": this.searchList[1].keyword,
          ...
          .then((response) => {
            if (response.data.errorMessage == "您还未登录，请先登录") {
              ...
              this.tableData = response.data.provCategorys
            }
          })
      ...
      handleEdit(index, row) {
        ...
        this.$router.push('/main/prov/'+ row.provCategoryId)
      },
      handleDelete(index, row) {
         this.$http.post(this.$api.Prov_delete,this.Qs.stringify({
            "provCategoryJ.provCategoryId":row.provCategoryId
          }))
        .then((response) => { 
          if(response.data.result == 1){
              this.tableData.splice(index, 1);
                this.$message({
                   message: '成功删除省份:  ' + row.provName,
                    type: 'success' 
                  })
      ...
      batchDelete() {
        ...
        this.multipleSelection.forEach(function(value, index, array) {
          ids.push(array[index].provCategoryId)
          console.log("ids",ids)
        })
        this.$http.post(this.$api.Prov_BatchDelete,
            this.Qs.stringify({
              "ids": ids
            }))
          .then((response) => {
      ...
      handleCurrentChange(page) {
        ...
        this.$http.post(this.$api.Prov_findAll,
        this.Qs.stringify({
          "page.pageNo":this.paginations.page_current,
          "page.pageSize":this.paginations.page_size
        }))
