    ...
    <div class="search_container">
      <el-form :inline="true"  class="demo-form-inline">
        ...
        <el-form-item style="float: right;">
          <router-link to="/main/contThird/add"><el-button type="primary" icon="el-icon-plus"></el-button></router-link>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{scope.row.categThreeId}}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="	名称">
...
export default {
  ...
  methods: {
    cataSearch() {
        ...
        this.$http.post(this.$api.Three_search,
            this.Qs.stringify({
              "CategoryT.categThreeId": this.searchList[0].keyword,
              "CategoryT.categName": this.searchList[1].keyword
            }))
          .then((response) => {
            if (response.data.errorMessage == "您还未登录，请先登录") {
              ...
              this.tableData = response.data.categoryThrees
            }
          })
      ...
      handleEdit(index, row) {
        ...
        this.$router.push('/main/contThird/'+ row.categThreeId)
      },
      handleDelete(index, row) {
         this.$http.post(this.$api.Three_delete,this.Qs.stringify({
            "CategoryT.categThreeId":row.categThreeId
          }))
        .then((response) => { 
          if(response.data.result == 1){
              this.tableData.splice(index, 1);
                this.$message({
                   message: '成功删除三级分类:  ' + row.categName,
                    type: 'success' 
                  })
      ...
      batchDelete() {
        ...
        this.multipleSelection.forEach(function(value, index, array) {
          ids.push(array[index].categThreeId)
          console.log("ids",ids)
        })
        this.$http.post(this.$api.Three_BatchDelete,
            this.Qs.stringify({
              "ids": ids
            }))
          .then((response) => {
      ...
      handleCurrentChange(page) {
        ...
        this.$http.post(this.$api.Three_findAll,
        this.Qs.stringify({
