    ...
    <div class="search_container">
      <el-form :inline="true"  class="demo-form-inline">
        ...
        <el-form-item style="float: right;">
          <router-link to="/main/contSecond/add"><el-button type="primary" icon="el-icon-plus"></el-button></router-link>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{scope.row.categSecondId}}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="	名称">
...
export default {
  ...
  methods: {
    cataSearch() {
        ...
        this.$http.post(this.$api.Second_search,
            this.Qs.stringify({
              "categoryS.categSecondId": this.searchList[0].keyword,
              "categoryS.categName": this.searchList[1].keyword
            }))
          .then((response) => {
            if (response.data.errorMessage == "您还未登录，请先登录") {
              ...
              this.tableData = response.data.categorySeconds
            }
          })
      ...
      handleEdit(index, row) {
        ...
        this.$router.push('/main/contSecond/'+ row.categSecondId)
      },
      handleDelete(index, row) {
         this.$http.post(this.$api.Second_delete,this.Qs.stringify({
            "categoryS.categSecondId":row.categSecondId
          }))
        .then((response) => { 
          if(response.data.result == 1){
              this.tableData.splice(index, 1);
                this.$message({
                   message: '成功删除二级分类:  ' + row.categName,
                    type: 'success' 
                  })
      ...
      batchDelete() {
        ...
        this.multipleSelection.forEach(function(value, index, array) {
          ids.push(array[index].categSecondId)
          console.log("ids",ids)
        })
        this.$http.post(this.$api.Second_BatchDelete,
            this.Qs.stringify({
              "ids": ids
            }))
          .then((response) => {
      ...
      handleCurrentChange(page) {
        ...
        this.$http.post(this.$api.Second_findAll,
        this.Qs.stringify({
