<template>
  <div class="page">
    <el-form ref="form" :rules="rules"  :model="form" label-width="120px">
        <el-form-item label="所属二级分类" ><!-- :label="group.label" -->
      ...
      <el-form-item label="分类模块">
        ...
        <el-form-item>
          <el-button type="primary" @click="submitForm('form')">添加三级分类</el-button>
...
export default {
  ...
  created() {
      this.GLOBAL.getCategoryS((data)=>{
              data = JSON.parse(JSON.stringify(data).replace(/categSecondId/g,"value"));
              data = JSON.parse(JSON.stringify(data).replace(/categName/g,"label"));
              this.form.catagory.option = data.categorySeconds
  ...
  methods:{
      submitForm(formName) {
            let data;
            this.$http.post(this.$api.Three_insert, this.Qs.stringify({
              "categoryT.categorySecondId": this.form.catagory.keyword,
              "categoryT.categName": this.form.categName,
              "categoryT.isTask": this.form.type.indexOf("任务分类") > -1?1:0,
              "categoryT.isService":  this.form.type.indexOf("服务分类") > -1?1:0,
              "categoryT.isHelp":  this.form.type.indexOf("帮助分类") > -1?1:0,
              "categoryT.isNews":  this.form.type.indexOf("新闻分类") > -1?1:0,
              "categoryT.winBidTaskFee": this.form.fee
            }))
            .then((response) => {
