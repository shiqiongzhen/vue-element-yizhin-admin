    ...
    <div class="search_container">
      <el-form :inline="true"  class="demo-form-inline">
        ...
        <el-form-item style="float: right;">
          <router-link to="/main/city/add"><el-button type="primary" icon="el-icon-plus"></el-button></router-link>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{scope.row.cityCategoryId}}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="	名称">
...
export default {
  ...
  methods: {
    cataSearch() {
        ...
        this.$http.post(this.$api.City_search,
            this.Qs.stringify({
              "cityCategoryJ.cityCategoryId": this.searchList[0].keyword,
              "cityCategoryJ.cityName": this.searchList[1].keyword,
          ...
          .then((response) => {
            if (response.data.errorMessage == "您还未登录，请先登录") {
              ...
              this.tableData = response.data.cityCategorys
            }
          })
      ...
      handleEdit(index, row) {
        ...
        this.$router.push('/main/city/'+ row.cityCategoryId)
      },
      handleDelete(index, row) {
         this.$http.post(this.$api.City_delete,this.Qs.stringify({
            "cityCategoryJ.cityCategoryId":row.cityCategoryId
          }))
        .then((response) => { 
          if(response.data.result == 1){
              this.tableData.splice(index, 1);
                this.$message({
                   message: '成功删除市:  ' + row.cityName,
                    type: 'success' 
                  })
      ...
      batchDelete() {
        ...
        this.multipleSelection.forEach(function(value, index, array) {
          ids.push(array[index].cityCategoryId)
          console.log("ids",ids)
        })
        this.$http.post(this.$api.City_BatchDelete,
            this.Qs.stringify({
              "ids": ids
            }))
          .then((response) => {
      ...
      handleCurrentChange(page) {
        ...
        this.$http.post(this.$api.City_findAll,
        this.Qs.stringify({
          "page.pageNo":this.paginations.page_current,
          "page.pageSize":this.paginations.page_size
        }))
