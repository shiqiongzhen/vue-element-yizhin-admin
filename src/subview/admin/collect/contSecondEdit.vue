...
export default {
  ...
  methods: {
      initData(page) {
        this.$http.post(this.$api.Second_search,this.Qs.stringify({
          "categoryS.categSecondId":this.$route.params.cataid
        }))
       .then((response) => { 
          if(response.data.errorMessage == "您还未登录，请先登录"){
              ...
              this.form = response.data.categorySeconds[0]
              if(response.data.categorySeconds[0].isTask) this.type.push("任务分类")
              if(response.data.categorySeconds[0].isService) this.type.push("服务分类")
              if(response.data.categorySeconds[0].isHelp) this.type.push("帮助分类")
              if(response.data.categorySeconds[0].isNews) this.type.push("新闻分类")
          }
        })
      ...
      submitForm(formName) {
            this.$http.post(this.$api.Second_update,
            this.Qs.stringify({
              "categoryS.categSecondId": this.$route.params.cataid,
              "categoryS.categName": this.form.categName,
              "categoryS.isTask": this.type.indexOf("任务分类") > -1?1:0,
              "categoryS.isService":  this.type.indexOf("服务分类") > -1?1:0,
              "categoryS.isHelp":  this.type.indexOf("帮助分类") > -1?1:0,
              "categoryS.isNews":  this.type.indexOf("新闻分类") > -1?1:0,
              "categoryS.winBidTaskFee": this.form.winBidTaskFee
            }))
            .then((response) => {
